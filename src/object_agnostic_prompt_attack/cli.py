"""Installed command-line interface for prompt training."""
...
from .training import run_training_pipeline, validate_dataset_inputs
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--config", default="configs/experiment.example.yaml")
...
def main(argv: list[str] | None = None) -> None:
    ...
    if args.validate_only:
        print(
            json.dumps(
                [validate_dataset_inputs(config, dataset) for dataset in datasets],
                indent=2,
                sort_keys=True,
            )
        )
    ...
    for result in run_training_pipeline(config, datasets):
        print(
